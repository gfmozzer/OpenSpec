...
function normalizeId(rawId: string): string {
  ...
  if (!ID_PREFIXES.has(prefix)) return rawId;
  const canonicalPrefix = prefix === 'RAD' ? 'EPIC' : prefix;
  return `${canonicalPrefix}-${digits.padStart(4, '0')}`;
}

function isLegacyId(rawId: string): boolean {
  const normalized = normalizeId(rawId);
  return normalized !== rawId;
}

function collectReferenceConversions(value: unknown, conversions: Map<string, string>): void {
  if (!value) return;
...
function replaceExactReferences(value: string, conversions: Map<string, string>): string {
  let next = value;
...
function traverseAndConvertReferences<T>(value: T, conversions: Map<string, string>): T {
  if (Array.isArray(value)) {
    return value.map((item) => traverseAndConvertReferences(item, conversions)) as T;
  }
  if (value && typeof value === 'object') {
    const updatedEntries = Object.entries(value as Record<string, unknown>).map(([key, entry]) => [
      key,
  ...
  if (typeof value === 'string') {
    return replaceExactReferences(value, conversions) as T;
  }
  return value;
}

async function readStateConfigVersion(configPath: string): Promise<number> {
  try {
    const raw = parseYaml(await fs.readFile(configPath, 'utf-8')) as Record<string, unknown> | null;
...
async function writeStateConfigVersion(configPath: string): Promise<void> {
  let payload: Record<string, unknown> = {};
  try {
    const parsed = parseYaml(await fs.readFile(configPath, 'utf-8')) as Record<string, unknown> | null;
...
async function safeRenameEntry(entryPath: string, nextPath: string): Promise<boolean> {
  try {
...
async function renameFilesystemEntries(rootDir: string, conversions: Map<string, string>): Promise<number> {
  ...
  for (const entryPath of targets) {
    ...
    if (nextPath === entryPath) continue;
...
async function rewriteFilesystemContents(rootDir: string, conversions: Map<string, string>): Promise<number> {
...
export async function assessSddMigration(projectRoot: string): Promise<SddMigrationAssessment> {
  ...
  if (snapshot.frontendGaps) collectReferenceConversions(snapshot.frontendGaps, conversions);
  if (snapshot.frontendMap) collectReferenceConversions(snapshot.frontendMap, conversions);
  if (snapshot.frontendDecisions) collectReferenceConversions(snapshot.frontendDecisions, conversions);
  ...
  if (currentVersion < CURRENT_SDD_STATE_VERSION) {
    reasons.push(
      `state_version=${currentVersion} esta abaixo da versao requerida ${CURRENT_SDD_STATE_VERSION}`
    );
  }
...
export class SddMigrateCommand {
  async execute(
    projectRoot: string,
    options?: { radToEpic?: boolean }
  ): Promise<{ converted: number; messages: string[]; assessment: SddMigrationAssessment }> {
    ...
    for (const [prefix, current] of Object.entries(snapshot.discoveryIndex.counters)) {
      ...
      snapshot.discoveryIndex.counters[prefix as keyof typeof snapshot.discoveryIndex.counters] = Math.max(
        current || 0,
        candidate
      );
    }
    ...
    snapshot.integrationContracts.contracts = traverseAndConvertReferences(
    ...
    if (snapshot.frontendGaps) {
      snapshot.frontendGaps.items = traverseAndConvertReferences(snapshot.frontendGaps.items, conversions).map(
        (item) => ({
          ...item,
          id: conversions.get(item.id) || item.id,
        })
    ...
    messages.push(
      `Migracao SDD concluida para state_version=${CURRENT_SDD_STATE_VERSION}. Referencias convertidas: ${converted}.`
    );
