export interface StructuralLens {
  ...
  section_rules?: Record<string, { min_lines?: number; min_length?: number }>;
}
...
export function extractSectionContent(content: string, sectionHeader: string): string {
...
function extractFromLines(lines: string[], startIndex: number): string {
  const contentLines = [];
...
export function validateDocumentAgainstLens(content: string, lens: StructuralLens): string[] {
  ...
  if (lens.section_rules) {
    for (const [sectionHeader, rules] of Object.entries(lens.section_rules)) {
      if (missing.includes(sectionHeader)) continue;
