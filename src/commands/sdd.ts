...
interface SddBreakdownCliOptions {
  titles?: string;
  scale?: 'QUICK' | 'STANDARD' | 'LARGE';
  mode?: 'graph' | 'flat';
  incremental?: boolean;
...
interface SddFinalizeCliOptions {
  ref?: string;
  allReady?: boolean;
  noAdr?: boolean;
  json?: boolean;
...
interface SddContextCliOptions {
  json?: boolean;
}

interface SddOnboardCliOptions {
  json?: boolean;
  compact?: boolean;
}
interface SddApproveCliOptions {
  etapa?: 'proposta' | 'planejamento' | 'tarefas';
  por?: string;
  observacao?: string;
  json?: boolean;
  render?: boolean;
...
interface SddSkillsSyncCliOptions {
  bundle?: string;
  all?: boolean;
  tools?: string;
}
...
interface SddGapDoneCliOptions {
  feature?: string;
  files?: string;
  routes?: string;
  render?: boolean;
...
export function registerSddCommand(program: Command): void {
  ...
  sddCmd
    ...
    .action(async (targetPath = '.', options?: SddInitContextCliOptions) => {
      ...
      console.log(
        `Detectado: stack=${result.contextBootstrap.detected.tech_stack_count}, repo-map=${result.contextBootstrap.detected.repo_map_count}, servicos=${result.contextBootstrap.detected.service_catalog_count}`
      );
      console.log(
        `Atualizado: stack=${result.contextBootstrap.updated.tech_stack ? 'sim' : 'nao'}, repo-map=${
          result.contextBootstrap.updated.repo_map ? 'sim' : 'nao'
        }, arquitetura=${result.contextBootstrap.updated.architecture ? 'sim' : 'nao'}, servicos=${
          result.contextBootstrap.updated.service_catalog ? 'sim' : 'nao'
        }, contratos=${result.contextBootstrap.updated.integration_contracts ? 'sim' : 'nao'}`
      );
  ...
  sddCmd
    ...
    .action(async (refOrText: string, options?: SddStartCliOptions) => {
      ...
      console.log(
        `Guardrails: blocked=${result.start_guardrails.blocked_check.ok ? 'ok' : 'fail'} | lock=${
          result.start_guardrails.lock_check.ok ? 'ok' : 'fail'
        } | forced=${result.start_guardrails.forced ? 'sim' : 'nao'}`
      );
      console.log(`Workspace ativo: ${result.active_path}`);
      console.log(`Docs gerados: ${result.generated_docs.join(', ')}`);
  ...
  sddCmd
    ...
    .action(async (ref: string, options?: SddContextCliOptions) => {
      ...
      if ((context as Record<string, unknown>).origin) {
        const origin = context.origin as { type: string; ref: string };
        console.log(`Origem: ${origin.type}${origin.ref ? ` (${origin.ref})` : ''}`);
      }
      if ((context as Record<string, unknown>).related_features) {
        const refs = context.related_features as string[];
        console.log(`Features relacionadas: ${refs.join(', ') || '-'}`);
      }
      if ((context as Record<string, unknown>).recommended_skills) {
        const skills = context.recommended_skills as string[];
  ...
  sddCmd
    ...
    .action(async (target = 'system', options?: SddOnboardCliOptions) => {
      ...
      if (Array.isArray(view.read_order)) {
        console.log(`Ordem de leitura: ${(view.read_order as string[]).join(' -> ')}`);
      }
      if (Array.isArray(view.proximos_passos)) {
        console.log(`Proximos passos: ${(view.proximos_passos as string[]).join(', ')}`);
      }
    });
