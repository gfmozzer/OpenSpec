...
describe('PowerShellGenerator', () => {
	...
	describe('generate', () => {
		...
		it('should register argument completer for opensdd command', () => {
			const commands: CommandDefinition[] = [
				{
					name: 'init',
					description: 'Initialize OpenSpec',
			...
			expect(script).toContain('Register-ArgumentCompleter -CommandName opensdd');
		...
		it('should generate dynamic completion helper for changes', () => {
			...
			expect(script).toContain('opensdd __complete changes 2>$null');
		...
		it('should generate dynamic completion helper for specs', () => {
			...
			expect(script).toContain('opensdd __complete specs 2>$null');
		});
