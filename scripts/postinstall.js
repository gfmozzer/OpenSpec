 ...
 * - OPENSDD_NO_COMPLETIONS=1 or OPENSPEC_NO_COMPLETIONS=1 environment variable is set
...
function shouldSkipInstallation() {
  ...
  if (process.env.OPENSDD_NO_COMPLETIONS === '1' || process.env.OPENSPEC_NO_COMPLETIONS === '1') {
    return { skip: true, reason: 'OPENSDD_NO_COMPLETIONS=1 or OPENSPEC_NO_COMPLETIONS=1 set' };
  }
...
async function installCompletions(shell) {
  try {
    ...
    if (!CompletionFactory.isSupported(shell)) {
      console.log(`\nTip: Run 'opensdd completion install' for shell completions`);
      return;
    }
