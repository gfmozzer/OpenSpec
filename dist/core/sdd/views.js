...
function renderFrontendGapsResolved(state) {
    const rows = (state.frontendGaps?.items ?? [])
        .filter((item) => item.status === 'DONE' || item.status === 'SUPERSEDED')
        .slice()
        .sort((a, b) => a.id.localeCompare(b.id))
...
function renderFrontendSitemap(state) {
    const routes = (state.frontendMap?.routes ?? [])
        .slice()
        .sort((a, b) => a.path.localeCompare(b.path));
    ...
    const rows = routes.map((route) => {
        ...
        const implementedFiles = route.implemented_files && route.implemented_files.length > 0
            ? route.implemented_files.join(', ')
            : '-';
...
function renderFrontendAudit(state) {
    const rows = state.backlog.items
        ...
        const surfaces = (item.frontend_surface_tokens || []).length > 0
...
export async function renderViews(paths, config, state) {
    ...
    if (config.frontend.enabled) {
        writes.push(fs.writeFile(path.join(paths.coreDir, 'frontend-map.md'), renderFrontendMap(state), 'utf-8'), fs.writeFile(path.join(paths.coreDir, 'frontend-sitemap.md'), renderFrontendSitemap(state), 'utf-8'), fs.writeFile(path.join(paths.coreDir, 'frontend-decisions.md'), renderFrontendDecisions(state), 'utf-8'), fs.writeFile(path.join(paths.pendenciasDir, 'frontend-gaps.md'), renderFrontendGaps(state), 'utf-8'), fs.writeFile(path.join(paths.pendenciasDir, 'frontend-gaps-resolvidos.md'), renderFrontendGapsResolved(state), 'utf-8'), fs.writeFile(path.join(paths.pendenciasDir, 'frontend-auditoria.md'), renderFrontendAudit(state), 'utf-8'));
    }
