...
const DEFAULT_META_EVOLUTION_CONFIG = {
    enabled: true,
...
function normalizePercent(part, total) {
    if (total <= 0)
        return 100;
    return Math.round((part / total) * 10000) / 100;
}
async function readMetaEvolutionConfig(paths) {
    try {
        const raw = parseYaml(await fs.readFile(paths.configFile, 'utf-8'));
...
function hasPlaceholder(content, markers) {
    const normalized = content.toLowerCase();
    return markers.some((marker) => normalized.includes(marker));
}
async function collectAuditArtifacts(paths) {
...
function debateHasRealDeliberation(content) {
    ...
    if (!escolhaMatch || !justificativaMatch)
        return false;
    ...
    const justificativaInvalid = invalidTokens.some((token) => justificativa === token || justificativa.includes(token));
    return !escolhaInvalid && !justificativaInvalid;
}
async function collectForcedTransitions(paths) {
    ...
    for (const root of roots) {
        const files = await listFilesRecursive(root);
        for (const filePath of files) {
            if (!filePath.endsWith('.md') || !/4-(changelog|historico)\.md$/i.test(filePath))
                continue;
...
function resolveRoutedSkills(snapshot, touches) {
    let injectedSkills = [];
    ...
    async execute(projectRoot, refOrText, options) {
        ...
        if (/^FEAT-\d{3,}$/.test(value)) {
            ...
            if (existing) {
                ...
                const shape = classifyFeatureShape(title);
                const recommended = resolveRoutedSkills(snapshot, shape.touches);
                feature = buildBacklogItem(id, title, 'epic', value, options?.scale || 'STANDARD', recommended, {
        ...
        if (!feature.change_name) {
            ...
            const canAdoptExistingChange = existsSync(existingChangeDir) &&
        ...
        if (feature.origin_type === 'epic' && feature.origin_ref) {
            ...
            const parentFile = files.find((f) => f.startsWith(`${feature.origin_ref}-`));
            if (parentFile) {
                ...
                if (content) {
                    ...
                    startLensViolations.push(...miss.map((m) => `Epic: ${m}`));
                }
                else {
...
export class SddAuditCommand {
    async execute(projectRoot) {
        ...
        const scoreBase = placeholdersMetric.percent * 0.4 + debateMetric.percent * 0.35 + adrMetric.percent * 0.25;
...
export class SddContextCommand {
    async execute(projectRoot, ref) {
        ...
        if (type === 'RAD' || type === 'EPIC') {
            ...
            if (!radar)
                throw new Error(`Epic/Radar ${ref} nao encontrado.`);
            const relatedFeatures = snapshot.backlog.items.filter((item) => (item.origin_type === 'radar' || item.origin_type === 'epic') && item.origin_ref === ref);
            return {
                context_pack_version: 1,
                target_id: ref,
                target_type: type,
